...
def corpus_entity_info(train_ner, test_ner):
    train_stat = train_ner.groupby('entity')['text'].agg(['count']).rename(columns={'count':'train'})
    test_stat = test_ner.groupby('entity')['text'].agg(['count']).rename(columns={'count':'test'})
    train_test_stat = pd.merge(train_stat, test_stat, on = 'entity', how='left').replace(np.nan, 0)
...
def spacy_data_conversion(dataset, mode = "train"):
    ...
    for text, ent in zip(dataset.text.values, dataset.entities.values):
        entitiy_list =  [(text.find(i.get('text')),text.find(i.get('text')) + len(i.get("text")) ,i.get('entity')) for i in ent]
        if mode == "train":
            ents = {}
